...
// Temporary, single-servo calibration sketch for J1 only.
// J2-J5 are deliberately never attached.
...
void processCommand(char *line) {
  ...
  if (strcmp(command, "PING") == 0 && argument == NULL) {
    Serial.println(F("PONG J1_CAL_V1"));
    return;
  }
  ...
  if (strcmp(command, "DETACH") == 0 && argument == NULL) {
    base.detach();
    attached = false;
    Serial.println(F("OK DETACHED J1"));
    return;
  }
...
void setup() {
  Serial.begin(115200);
  Serial.println(F("READY J1_CAL_V1 DETACHED"));
}
